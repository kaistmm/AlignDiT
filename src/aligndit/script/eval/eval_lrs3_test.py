...
from aligndit.script.eval.utils import get_lrs3_test, run_asr_wer, run_avsync
...
def get_args():
    ...
    parser.add_argument("-e", "--eval_task", type=str, default="wer", choices=["sim", "wer", "avsync"])
    parser.add_argument("-l", "--lang", type=str, default="en")
    parser.add_argument("-g", "--gen_wav_dir", type=str, required=True)
    parser.add_argument("-n", "--gpu_nums", type=int, default=8, help="Number of GPUs to use")
    parser.add_argument("--gt_av_feat", type=str, default="data/LRS3_debug/autoavsr/avhubert_feat")
    parser.add_argument("--gen_av_feat", type=str, default="")
...
def main():
    ...
    if eval_task == "wer":
        ...
        with mp.Pool(processes=len(gpus)) as pool:
            args = [(rank, sub_test_set, args.gt_av_feat, args.gen_av_feat) for (rank, sub_test_set) in test_set]
            results = pool.map(run_avsync, args)
            for r in results:
                full_results.extend(r)
